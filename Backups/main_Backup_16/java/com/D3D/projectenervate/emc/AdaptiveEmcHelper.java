...
public final class AdaptiveEmcHelper {
    ...
    public static void applyMergedEconomicState(
        ...
        BigDecimal baseSingle = AdaptiveEmcOutputHelper.getBaseSingleEmc(mergedStack);

        if (combinedTotal.signum() > 0) {
            ...
            if (baseSingle.signum() <= 0 || perItem.compareTo(baseSingle) > 0) {
                if (baseSingle.signum() > 0 && ProjectEnervateConfig.chooseBaseEmcIfLower()) {
                    ProjectEnervateSourceHelper.markVerified(mergedStack);
                } else {
